...
6. Missing sensors degrade gracefully rather than promoting HR-only triggers. Recorder data can lag by up to ~3 minutes, live motion can go stale, and HR cadence is irregular when no workout is active. Early wake decisions still require motion evidence; if live motion is unavailable, Smart Wake preserves exact-wake haptics, local light fallback, and the HomeKit scene rather than firing from HR alone.
7. `WakeHeuristicEngine` keeps 1-second motion bins plus 5s/30s/60s/180s rolling windows and time-aware HR windows. It computes `motionEnergy60s`, `motionBurst30s`, `rotationVariance60s`, `postureShift300s`, `stillnessBreakScore`, `hrDelta`, `hrSlope180s`, `hrFreshnessPenalty`, and `motionFreshnessPenalty`. Composite confidence weights are motion arousal `0.55`, motion-vs-baseline `0.20`, HR arousal `0.20`, and proximity-to-wake prior `0.05`. The first 5 minutes of the wake window use a higher threshold, the final 10 minutes relax it slightly, and the final 3 minutes can trigger on strong motion-only evidence when HR is stale.
